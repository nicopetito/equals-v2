-- Actualiza rpc_transactions_batch_insert para aceptar y propagar transaction_kind.
-- Si no se provee, se infiere del tipo (income → 'income', expense → 'expense').
-- Esto permite que importaciones CSV creen transacciones con el kind correcto.
...
BEGIN
  ...
  SELECT
    ...
    (t->>'notes')::text,
    COALESCE(
      (t->>'transaction_kind')::public.transaction_kind_enum,
      CASE (t->>'type')::text
        WHEN 'income'  THEN 'income'::public.transaction_kind_enum
