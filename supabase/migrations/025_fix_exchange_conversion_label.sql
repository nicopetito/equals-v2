-- Corrige rpc_exchange_conversion para que las conversiones de divisas
-- no inflen ingresos/egresos en estadísticas.
-- El label 'internal_transfer' es excluido por INTERNAL_LABELS en el cliente.
...
BEGIN
  ...
  VALUES
    (v_user_id, p_description, p_from_amount, 'expense', p_from_currency, p_from_wallet_id, p_date, 'internal_transfer', v_notes, false)
  RETURNING id INTO v_leg1_id;

  INSERT INTO public.transactions
    (user_id, description, amount, type, currency, wallet_id, date, label, notes, is_recurring)
  VALUES
    (v_user_id, p_description, p_to_amount, 'income', p_to_currency, p_to_wallet_id, p_date, 'internal_transfer', v_notes, false)
