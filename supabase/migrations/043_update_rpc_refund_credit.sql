-- Actualiza rpc_refund_credit para setear transaction_kind='refund_credit'.
-- Antes: la transacción de ingreso creada por un reintegro no tenía ningún label ni kind,
-- y solo se identificaba por el prefijo "Reintegro: " en la descripción (frágil).
-- Ahora: transaction_kind='refund_credit' permite filtrarla formalmente en reportes.
...
BEGIN
  ...
  INSERT INTO public.transactions
    (user_id, description, amount, type, currency, wallet_id, date, notes, transaction_kind)
  VALUES
    (v_user_id, 'Reintegro: ' || v_orig_desc, v_amount, 'income', v_currency,
     v_wallet_id, CURRENT_DATE, v_note, 'refund_credit')
