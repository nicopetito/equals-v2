-- Actualiza rpc_wallet_transfer para setear transaction_kind='transfer'
-- y transfer_group_id en ambas transacciones del par (expense + income).
-- El transfer_group_id es el mismo UUID que el transfer_id existente.
...
BEGIN
  ...
  VALUES
    (v_user_id, p_description, p_amount, 'expense', p_currency, p_from_wallet_id, p_date,
     'internal_transfer', v_notes, false, 'transfer', v_transfer_id)
  ...
  INSERT INTO public.transactions
    (user_id, description, amount, type, currency, wallet_id, date,
     label, notes, is_recurring, transaction_kind, transfer_group_id)
  VALUES
    (v_user_id, p_description, p_amount, 'income', p_currency, p_to_wallet_id, p_date,
     'internal_transfer', v_notes, false, 'transfer', v_transfer_id)
